...
>
	<section class="mb-16">
		<h3 class="text-xl font-bold text-white mb-6 gap-3 flex items-center">
			<LessonIcon name="TestTube" size={20} class="text-green-400" />
			Basic Unit Test
		</h3>
		<p class="text-sm text-white/70 leading-relaxed mb-8">
			Testing a plain TypeScript function is the simplest way to start. We import <code
				class="px-1.5 py-0.5 rounded bg-white/10 text-white font-mono text-xs">describe</code
			>, <code class="px-1.5 py-0.5 rounded bg-white/10 text-white font-mono text-xs">it</code>, and
			<code class="px-1.5 py-0.5 rounded bg-white/10 text-white font-mono text-xs">expect</code> from
			Vitest.
		</p>

		<div class="lg:grid-cols-2 gap-6 mb-12 grid grid-cols-1">
		...
		<!-- Test Simulation -->
		<div
			class="p-8 rounded-xl border-white/10 bg-white/5 glass-blur space-y-6 relative overflow-hidden border"
		>
			<div class="-top-24 -right-24 w-64 h-64 bg-green-500/5 blur-3xl absolute rounded-full"></div>
			...
			<div class="relative z-10 flex items-center justify-between">
				...
				<div class="gap-2 flex items-center">
					<div
						class="w-1.5 h-1.5 rounded-full {testResult === 'success'
			...
			{#if testResult === 'success'}
				<div
					class="p-6 rounded-xl bg-black/40 border-green-500/20 space-y-3 animate-fade-in glass-blur relative z-10 border"
				>
					<div class="gap-2 text-green-400 flex items-center">
						<CheckCircle size={16} />
						<span class="text-xs font-bold tracking-widest uppercase">PASS</span>
						<span class="text-xs font-mono opacity-60">src/lib/greet.spec.ts</span>
					</div>
					<div class="pl-6 border-white/10 text-white/50 space-y-1 border-l text-[11px]">
						<div class="gap-2 flex items-center">
							<span class="text-green-500">✓</span>
							<span>greet &gt; returns a greeting (2ms)</span>
						</div>
						<div class="pt-2 tracking-tighter text-[10px] uppercase opacity-30">
	...
	<section class="mb-16">
		...
		<p class="text-sm text-white/70 leading-relaxed mb-8">
			To test Svelte components, we use <code
				class="px-1.5 py-0.5 rounded bg-white/10 text-white font-mono text-xs"
				>@testing-library/svelte</code
			>. It allows us to render components in a simulated DOM and query for elements just like a
			user would.
		</p>
		<CodeBlock code={svelteSpecCode} lang="typescript" filename="Welcome.spec.ts" />

		<div class="mt-8 p-6 rounded-xl bg-white/5 border-white/10 gap-4 glass-blur flex border">
			<div class="p-2 rounded-lg h-fit bg-(--color-accent)/10 text-(--color-accent)">
				<AlertCircle size={18} />
			</div>
			<div>
				<strong class="text-sm text-white mb-1 tracking-widest block text-[10px] uppercase"
					>Pro Tip</strong
				>
				<p class="text-xs text-white/50 leading-relaxed">
					Use <code class="px-1 py-0.5 rounded bg-white/10 text-white font-mono"
						>npm run test:unit</code
					> to run Vitest in watch mode while developing. This provides instant feedback as you save files.
				</p>
			</div>
		</div>
	</section>

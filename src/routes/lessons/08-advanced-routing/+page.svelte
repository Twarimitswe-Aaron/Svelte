	...
	const topics = [
		{
			...
			fails:
				'Without a catch-all, any unmatched deep URL returns SvelteKit\'s default "Not found" page instead of your custom one.',
...
// src/routes/[...404]/+error.svelte catches all unmatched routes`
		},
		{
			...
			fails:
				'Without optional params, you need two separate routes: /blog and /[lang]/blog — duplicating code.',
...
export const load = ({ params }) => ({
  lang: params.lang ?? 'en' // defaults to 'en' if not provided
		...
		},
		{
			...
			fails:
				"Without a matcher, /users/abc matches [id] even if your app only works with numeric IDs — you'd need to validate inside load().",
...
export const load = ({ params }) => ({
  userId: parseInt(params.id) // id is guaranteed to be numeric
		...
		},
		{
			...
			fails:
				"Without groups, you'd either add unnecessary URL segments or duplicate the auth layout code across many pages.",
...
// Can check auth and redirect if not logged in`
		},
		{
			...
			fails:
				'Without @, a full-screen landing page inside a dashboard group would be wrapped in the dashboard nav — ugly and structurally wrong.',
...
>
	<!-- Topic tabs -->
	<div class="gap-2 mb-8 flex flex-wrap">
		{#each topics as topic (topic.id)}
			<button
				class="px-4 py-2 rounded-xl text-sm font-medium gap-2 flex items-center border transition-all {selected.id ===
				topic.id
	...
	{#key selected.id}
		<div class="p-8 rounded-xl border-white/10 bg-white/5 glass-blur animate-fade-in border">
			<div class="mb-8 pb-8 border-white/10 border-b">
				<h3 class="mb-3 font-bold text-white gap-3 flex items-center text-[0.95rem]">
					<span class="text-2xl">{selected.emoji}</span>
					{selected.title}
				</h3>
				<p class="mb-3 text-[0.875rem] leading-[1.65] text-(--color-text-muted)">{selected.desc}</p>
			</div>

			<div class="md:grid-cols-2 gap-6 mb-10 grid grid-cols-1">
				<div class="p-6 rounded-xl border-white/5 bg-white/2 space-y-3 glass-blur border">
					...
					<p class="text-[0.875rem] leading-[1.65] text-(--color-text-muted)">{selected.what}</p>
				</div>
				<div class="p-6 rounded-xl border-white/5 bg-white/2 space-y-3 glass-blur border">
					<span
						class="font-bold tracking-widest text-white/30 gap-2 flex items-center text-[10px] uppercase"
					...
					<p class="text-[0.875rem] leading-[1.65] text-(--color-text-muted)">{selected.fails}</p>
				</div>
			</div>
	...
	<p class="mb-3 text-[0.875rem] leading-[1.65] text-(--color-text-muted)">
		The route <code class="px-1.5 py-0.5 rounded bg-white/10 text-white font-mono"
			>08-advanced-routing/[...path]</code
		> catches any sub-URL. Try these:
	</p>
	<div class="mb-4 gap-1.5 flex flex-col">
		{#each ['/lessons/08-advanced-routing/foo', '/lessons/08-advanced-routing/foo/bar/baz', '/lessons/08-advanced-routing/any/deep/path'] as link (link)}
			...
			>
				<code class="text-xs pr-4 truncate">{link}</code>
			</a>
		{/each}
	</div>
</LessonLayout>

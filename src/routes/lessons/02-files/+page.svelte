<script lang="ts">
	...
	import { CheckCircle2, XCircle, FileJson } from 'lucide-svelte';
...
>
	<div class="mb-12">
		...
		<div class="lg:grid-cols-[320px_1fr] gap-8 grid grid-cols-1">
			<!-- Sidebar Selection -->
			<div class="gap-3 flex flex-col">
				{#each fileTypes as file (file.id)}
					<button
						class="group gap-4 rounded-xl p-4 flex items-center border text-left transition-all duration-300 {activeId ===
						file.id
					...
					>
						<div
							class="h-11 w-11 rounded-xl bg-black/40 flex shrink-0 items-center justify-center {file.color} transition-transform group-hover:scale-105"
						>
							<LessonIcon name={file.icon} size={22} />
						</div>
						<div class="flex flex-col">
							<span
								class="font-bold tracking-tight text-[13px] transition-colors {activeId === file.id
									? 'text-white'
									: 'text-white/40 group-hover:text-white/60'}">{file.name}</span
							>
							<span class="font-bold tracking-widest text-[10px] uppercase opacity-20"
								>{file.title}</span
							>
						</div>
					</button>
			...
			<div class="relative min-h-[500px]">
				{#key activeId}
					<div class="animate-fade-in space-y-6">
						<div
							class="p-8 rounded-xl border-white/10 bg-white/5 glass-blur space-y-8 backdrop-blur-md border"
						>
							<div
								class="md:flex-row md:items-center gap-6 pb-8 border-white/5 flex flex-col justify-between border-b"
							>
								<div>
									<h4 class="text-2xl font-black text-white tracking-tight mb-1">
										{activeFile.name}
									</h4>
									<p class="text-sm text-white/50 leading-relaxed italic">{activeFile.role}</p>
								</div>
								<div
									class="px-4 py-2 rounded-xl bg-white/5 border-white/10 font-bold tracking-widest text-white/40 glass-blur h-fit border text-[10px] uppercase"
								>
							...
							<div class="md:grid-cols-2 gap-6 grid grid-cols-1">
								<div
									class="p-6 rounded-xl bg-green-500/5 border-green-500/10 space-y-3 glass-blur border"
								>
									<div
										class="gap-2 font-bold text-green-400 tracking-widest flex items-center text-[10px] uppercase"
									>
								...
								<div
									class="p-6 rounded-xl bg-red-500/5 border-red-500/10 space-y-3 glass-blur border"
								>
									<div
										class="gap-2 font-bold text-red-400 tracking-widest flex items-center text-[10px] uppercase"
									>
							...
							<div class="space-y-4 pt-4">
								<div
									class="gap-2 font-bold text-white/20 tracking-widest flex items-center text-[10px] uppercase"
								>
	...
	<!-- Comparison Section -->
	<div
		class="mt-16 p-10 rounded-xl border-white/10 bg-white/5 glass-blur relative overflow-hidden border"
	>
		<div class="-bottom-24 -left-24 w-64 h-64 bg-yellow-500/5 blur-3xl absolute rounded-full"></div>

		<h3 class="text-xl font-bold text-white mb-10 gap-3 relative z-10 flex items-center">
			<LessonIcon name="Zap" size={20} class="text-yellow-400" />
			Universal vs. Server-Only Loading
		</h3>

		<div class="md:grid-cols-2 gap-12 relative z-10 grid grid-cols-1">
			<div class="space-y-6">
				<div class="gap-3 text-yellow-400 flex items-center">
					<LessonIcon name="Zap" size={18} />
					<h4 class="text-sm font-bold tracking-widest uppercase">+page.ts (Universal)</h4>
				</div>
				<ul class="space-y-4">
					<li class="gap-3 group flex items-start">
						<div
							class="mt-1.5 h-1.5 w-1.5 bg-yellow-500/40 group-hover:bg-yellow-500 rounded-full transition-colors"
						></div>
						<p class="text-sm text-white/50 group-hover:text-white/80 transition-colors">
							Best for public APIs or data that should be fetched on the client during SPA
							navigation.
						</p>
					</li>
					<li class="gap-3 group flex items-start">
			...
			<div class="space-y-6">
				<div class="gap-3 text-red-400 flex items-center">
					<LessonIcon name="Server" size={18} />
					<h4 class="text-sm font-bold tracking-widest uppercase">+page.server.ts (Server-only)</h4>
				</div>
				<ul class="space-y-4">
					<li class="gap-3 group flex items-start">
						<div
							class="mt-1.5 h-1.5 w-1.5 bg-red-500/40 group-hover:bg-red-500 rounded-full transition-colors"
						></div>
						<p class="text-sm text-white/50 group-hover:text-white/80 transition-colors">
